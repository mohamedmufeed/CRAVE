...
const loadUserMangment = async (req, res) => {
  try {
    ...
    res
      ...
      .send("An error occurred while loading users.");
  }
};
...
const getSalesReport = async (time, startDate, endDate) => {
  let pipeline = [{ $match: { status: "Delivered" } }];
...
const generatePDFReport = async (req, res) => {
  ...
  try {
    ...
    doc
      ...
      .font("Helvetica")
      .text(
        `Total Sales Revenue: MRP: ₹${overallSummary.overallOrderAmount.toFixed(
          2
        )}`,
        { align: "left" }
      );
    doc.text(
      `Total Discount Given: MRP: ₹${overallSummary.overallDiscount.toFixed(
        2
      )}`,
      { align: "left" }
    );
    ...
    doc
      .fontSize(8)
      .text("Generated by Crave Report System", { align: "center" });
